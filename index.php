...
<div style="margin-top: 50px;" class="modal fade modal-horarios" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-sm">
		...
		<?php
		$lerbanco->ExeRead('delivery_distance_config', 'WHERE user_id = :client', "client={$getu}");
		if ($lerbanco->getResult()):
		...
		?>
			...
			elseif ($delivery_distance_config_result[0]['type'] == 'distance_rate'):
				$lerbanco->ExeRead('delivery_distance_rate', 'WHERE user_id = :client ORDER BY distance_rate', "client={$getu}");
				if ($lerbanco->getResult()):
				?>
					<b style="color: #61617a;"><i class="icon_pin"></i>  Entrega por distância </b>
					<br />
					<br />
					<table style="width: 250px;" data-sortable class="table">
						<thead>
			...
			<h3 style="color: #ffffff;">    
				...
				if($hr >= 12 && $hr<18) {
					$resp = "Boa tarde!";}
						...
						<p>Clique em um de nossos Administradores e Converse com eles!</p></div>
