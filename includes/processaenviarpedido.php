...
if(isset($get_dados_pedido['enviar_pedido']) && $get_dados_pedido['enviar_pedido'] == 'enviar_agora'):
	...
	elseif(strlen($get_dados_pedido['telefone']) < 11):
		...
		var sound = new Howl({
			...
			$get_dados_pedido['resumo_pedidos']   = $pedidos == "" ? "<b>Qtd:</<br /><b>Valor:</b> R$ ".Check::Real($get_dados_pedido['valor_taxa']+$get_dados_pedido['sub_total'])."..." : $pedidos;
		...
		//$enviarPedidos = str_replace('Qtd:', '%0AQtd:', $enviarPedidos);
			...
			if($get_dados_pedido['opcao_delivery'] != 'false'):								
				...
				if (isset($get_dados_pedido['bairro2']) || isset($get_dados_pedido['bairro'])):
			...
			if($get_dados_pedido['opcao_delivery'] == 'true'):
				if (isset($bairrolink) && !empty($bairrolink)):
					...
					$terceira_parte_pedido = "*Endereço:*<br />Rua: {$get_dados_pedido['rua']}, Nº: {$get_dados_pedido['unidade']},<br />Cidade: {$get_dados_pedido['cidade']}, {$get_dados_pedido['uf']},<br />Complemento:<br />{$get_dados_pedido['complemento']}<br />OBS: {$get_dados_pedido['observacao']}<br /><br />";
				endif;
